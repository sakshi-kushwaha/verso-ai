...
def _generate_edge_tts(text: str, path: Path, voice: str) -> bool:
    ...
    try:
        ...
        if word_times:
            ...
            # Write empty list so cache check passes
            with open(ts_path, "w", encoding="utf-8") as f:
                json.dump([], f)
...
def get_word_timestamps(audio_path: str | Path) -> list[dict] | None:
    ...
    Empty lists (from Piper/espeak fallback) return None so callers
    fall back to uniform word-rate estimation.
    """
...
def generate_audio(text: str, reel_index: int = 0) -> Path:
    ...
    with _tts_lock:
        ...
        if _generate_piper(text, piper_path, speaker_id=piper_speaker):
            ...
            # Write empty timestamps so cache check passes on next call
            with open(ts_path, "w", encoding="utf-8") as f:
                json.dump([], f)
